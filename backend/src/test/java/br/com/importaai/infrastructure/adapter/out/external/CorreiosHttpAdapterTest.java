...
class CorreiosHttpAdapterTest extends MessagingIntegrationTest {
    ...
    private void stubSucesso() {
        WIREMOCK.stubFor(get(urlPathMatching("/rastreamento/.*"))
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody("""
                                {"eventos":[{"tipo":"NA_CHINA","data":"2026-05-01T10:00:00Z",
                                "local":"Shenzhen","descricao":"postado"}]}""")));
    }

    private void stubFalha() {
        WIREMOCK.stubFor(get(urlPathMatching("/rastreamento/.*"))
                .willReturn(aResponse().withStatus(500)));
    }

    private int requestsRecebidas() {
        return WIREMOCK.findAll(getRequestedFor(urlPathMatching("/rastreamento/.*"))).size();
    }
    ...
    void tc30_circuitBreakerAbreEUsaCache() {
        ...
        // 1 sucesso popula o cache
        stubSucesso();
        List<EtapaRastreamento> primeira = port.consultar("BR123", criadoEm);
        ...
        int antes = requestsRecebidas(); // 1 sucesso + 5 falhas = 6
        assertThat(antes).isEqualTo(6);

        // proximas chamadas: circuito aberto -> NAO toca a API, retorna do cache
        List<EtapaRastreamento> doCache = port.consultar("BR123", criadoEm);
        port.consultar("BR123", criadoEm);

        assertThat(requestsRecebidas()).isEqualTo(antes); // continua 6: curto-circuito
