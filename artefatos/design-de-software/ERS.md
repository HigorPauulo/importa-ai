...
| RN09 | Limite de Notificações | O sistema mantém no máximo 50 notificações por usuário no histórico. Notificações mais antigas são removidas automaticamente (FIFO) quando o limite é atingido. |
...
Os detalhes de implementação que apoiam as regras de negócio acima foram movidos para os documentos apropriados, em alinhamento com a separação "o que (ERS) × como (arquitetura/design)":

- **Modelo de dados** (tabelas que suportam RF03, RN04, RN09 e RNF06): [`artefatos/modelagem-de-dados/modelo-de-dados.md`](../modelagem-de-dados/modelo-de-dados.md)
- **Estratégia de adapters para a API dos Correios** (apoio a RF12 e RF15): [ADR-004](adrs/004-adapters-correios.md)
- **Arquitetura de mensageria** (idempotência, retry, DLQ — suportes a RN04 e RN05): [`artefatos/mensageria-e-streams/arquitetura-mensageria.md`](../mensageria-e-streams/arquitetura-mensageria.md)
- **Decisões arquiteturais** (broker, Hexagonal, status derivado): [ADRs](adrs/)
